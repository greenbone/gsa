...
export const createScheduleInput = (creationDate, startDate, startTimezone) => {
  const event = Event.fromData({startDate}, startTimezone);

  return {
    input: {
      name: `Schedule for myFirstTask - ${creationDate.toISOString()}`,
...
export const createWizardScheduleQueryMock = (
  ...
  createGenericQueryMock(
    ...
    createScheduleInput(creationDate, startDate, startTimezone),
    errors,
  );
...
export const createAlertInput = creationDate => {
  return {
    input: {
      name: `Email Alert for myFirstTask - ${creationDate.toISOString()}`,
      comment: 'Automatically generated by wizard',
