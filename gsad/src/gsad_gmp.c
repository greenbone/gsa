...
static char *
edit_task (gvm_connection_t *, credentials_t *, params_t *, const char *,
           cmd_response_data_t *);
...
setting_get_value_error (gvm_connection_t *connection,
                         credentials_t *credentials, const gchar *setting_id,
                         gchar **value, cmd_response_data_t *response_data)
{
          ...
          cmd_response_data_t *response_data)
{
  ...
  ret = setting_get_value (connection, "f9f5a546-8018-48d0-bef5-5ad4926ea899",
                           &alert, response_data);
  if (ret)
    {
      switch (ret)
  ...
  PARAM_OR_SETTING (schedule, "schedule_id",
                    "778eedad-5550-4de0-abb6-1320d13b5e18", g_free (alert));

  PARAM_OR_SETTING (target, "target_id", "23409203-940a-4b4a-b70c-447475f18323",
                    g_free (alert);
                    g_free (schedule));

  PARAM_OR_SETTING (openvas_config, "config_id",
                    "fe7ea321-e3e3-4cc6-9952-da836aae83ce", g_free (alert);
                    g_free (schedule); g_free (target));

  PARAM_OR_SETTING (osp_config, "osp_config_id",
                    "fb19ac4b-614c-424c-b046-0bc32bf1be73", g_free (alert);
                    g_free (schedule); g_free (target);
                    g_free (openvas_config));

  PARAM_OR_SETTING (openvas_scanner, "scanner_id",
                    "f7d0f6ed-6f9e-45dc-8bd9-05cced84e80d", g_free (alert);
                    g_free (schedule); g_free (target); g_free (openvas_config);
                    g_free (osp_config));

  PARAM_OR_SETTING (osp_scanner, "osp_scanner_id",
                    "b20697c9-be0a-4cd4-8b4d-5fe7841ebb03", g_free (alert);
                    g_free (schedule); g_free (target); g_free (openvas_config);
                    g_free (osp_config); g_free (openvas_scanner));
  ...
  g_string_append_printf (
    ...
    osp_config ? osp_config : "", openvas_scanner ? openvas_scanner : "",
    osp_scanner ? osp_scanner : "", schedule ? schedule : "",
  ...
  /* Get list of targets. */
  if (gvm_connection_sendf (connection, "<get_targets"
  ...
  if (gvm_connection_sendf (connection, "<get_configs"
                                        " filter=\"rows=-1 sort=name\"/>")
      == -1)
    {
      g_string_free (xml, TRUE);
  ...
  if (command_enabled (credentials, "GET_ALERTS"))
    {
      ...
      if (gvm_connection_sendf (connection, "<get_alerts"
                                            " filter=\"rows=-1 sort=name\"/>")
          == -1)
        {
          g_string_free (xml, TRUE);
  ...
  if (command_enabled (credentials, "GET_SCHEDULES"))
    {
      ...
      if (gvm_connection_sendf (connection, "<get_schedules"
                                            " filter=\"rows=-1 sort=name\"/>")
          == -1)
        {
          g_string_free (xml, TRUE);
  ...
  if (command_enabled (credentials, "GET_SCANNERS"))
    {
      ...
      if (gvm_connection_sendf (connection, "<get_scanners"
                                            " filter=\"rows=-1\"/>")
          == -1)
        {
          g_string_free (xml, TRUE);
  ...
  if (command_enabled (credentials, "GET_GROUPS"))
    {
      ...
      if (gvm_connection_sendf (connection, "<get_groups/>") == -1)
        {
          g_string_free (xml, TRUE);
          cmd_response_data_set_status_code (response_data,
                                             MHD_HTTP_INTERNAL_SERVER_ERROR);
  ...
  g_string_append_printf (xml,
                          ...
                          "<alerts>%s</alerts>"
                          "</new_task>",
                          apply_overrides, alerts ? alerts : "1");
           ...
           cmd_response_data_t *response_data)
{
  ...
  if (gvm_connection_sendf (
        ...
        command_enabled (credentials, "GET_SCHEDULES")
          ? "<get_schedules"
            " filter=\"rows=-1 sort=name\"/>"
          : "",
        command_enabled (credentials, "GET_SCANNERS") ? "<get_scanners"
                                                        " filter=\"rows=-1\"/>"
                                                      : "",
        command_enabled (credentials, "GET_GROUPS") ? "<get_groups/>" : "")
      == -1)
    {
      cmd_response_data_set_status_code (response_data,
                                         MHD_HTTP_INTERNAL_SERVER_ERROR);
  ...
  g_string_append_printf (xml,
                          ...
                          "<apply_overrides>%i</apply_overrides>",
                          task_id, alerts ? alerts : "1", sort_field,
                          sort_order, apply_overrides);
