    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <script type="text/javascript">
          gsa.set_token("<xsl:value-of select="gsa:escape-js (/envelope/token)"/>");
        </script>
   ...
   <ul>
    ...
    <li>
      <xsl:variable name="items" xmlns="">
        ...
        <xsl:if test="gsa:may-op ('GET_RESULTS')">
          <item>
            <url>/ng/vulnerabilities</url>
            <name><xsl:value-of select="gsa:i18n ('Vulnerabilities')"/></name>
          </item>
        </xsl:if>
        <divider/>
