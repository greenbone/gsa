...
describe('NotesActiveDaysDisplay', () => {
  ...
  test('should be registered with the correct title', () => {
    const registered = getDisplay(NotesActiveDaysDisplay.displayId);

    expect(registered?.component).toBe(NotesActiveDaysDisplay);
    expect(String(registered?.title)).toBe('Chart: Notes by Active Days');
  });
...
describe('NotesActiveDaysTableDisplay', () => {
  test('should export a valid component with the correct configuration', () => {
    expect(NotesActiveDaysTableDisplay).toBeDefined();
    expect(NotesActiveDaysTableDisplay.displayId).toBe(
      'note-by-active-days-table',
    );
    expect(NotesActiveDaysTableDisplay.displayName).toBe(
      'NotesActiveDaysTableDisplay',
    );
  });

  test('should be registered with the correct title', () => {
    const registered = getDisplay(NotesActiveDaysTableDisplay.displayId);

    expect(registered?.component).toBe(NotesActiveDaysTableDisplay);
    expect(String(registered?.title)).toBe('Table: Notes by Active Days');
  });

  test('should render the configured table data', async () => {
    renderDisplay(<NotesActiveDaysTableDisplay height={200} width={200} />);
