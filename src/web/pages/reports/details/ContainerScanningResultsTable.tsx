...
const getColumns = (enableEPSS = false, onToggleDetailsClick?: () => void) => [
  ...
  },
  {
    ...
    render: (entity: Result) => {
      ...
      const sha = host?.name;
      const displayName = isDefined(image) && image !== '' ? image : sha;
  ...
  },
  {
    ...
    render: (entity: Result) => {
      ...
      const shortenedName = shorten(hostname, 40);
      return <span title={hostname}>{shortenedName}</span>;
    },
  },
  ...(enableEPSS
    ? [
...
const Header = ({
  ...
  return (
    <TableHeader>
      <TableRow>
        {columns.map(column => {
          if (column.headerGroup || (column.key === 'created' && hasEPSS)) {
            return null;
          }
          return (
            <TableHead
