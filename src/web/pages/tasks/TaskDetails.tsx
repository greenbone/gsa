...
const TaskDetails = ({entity, links = true}: TaskDetailsProps) => {
  ...
  const scanConfig = useShallowEqualSelector(state =>
    scanConfigSelector(state).getEntity(entity.config?.id),
  ) as ScanConfig | undefined;
  const schedule = useShallowEqualSelector(state =>
    scheduleSelector(state).getEntity(entity.schedule?.id),
