...
`;
  ...
  test('should parse XML with many entities without hitting entity expansion limits', () => {
    // Production XML responses can contain thousands of XML entities (&lt;, &gt;, &amp;, etc.)
    ...
    const response = new Response<string, Meta>(mockXml, {});
    ...
    const transformedResponse = transform.success(response);
    expect(typeof transformedResponse.data.report).toBe('object');
  });

  test('should parse deeply nested XML with many entities', () => {
    // Simulate a complex report structure with many entity references
    // This tests the scenario of large scan reports with extensive data

    const createResult = (id: number) => `
      <result id="${id}">
        <name>Test &lt;${id}&gt;</name>
        <description>Found &amp; detected &quot;issue&quot;</description>
    ...
    const mockXml = createEnvelopedXml(
      `<get_reports_response><report>${results}</report></get_reports_response>`,
    );
