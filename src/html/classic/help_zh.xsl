...
<xsl:template mode="help" match="users.html">
  ...
  <div class="gb_window_part_content">
    ...
    <div style="text-align:left">
      ...
      <h2>LDAP 认证和授权</h2>
      <p>       
       这个对话框仅对这种情况可见，即后台被配置成支持 LDAP 认证和授权。
      </p>
      <p>       
       使用这种方式，需要在 LDAP 后台做大量工作，比如访问规则和角色存储在 LDAP 里，
       这与 "per-User" 方式相反。建议还是使用 "per-User" 方式。
      </p>
      <p>       
       更改将会在点击 “保存” 按钮确认后被保存，但是<b>仅在后台服务重启后才会生效</b>。
      </p>
      <table class="gbntable">
        ...
        <tr class="even">
          <td>LDAP Host（LDAP主机）</td>
          <td>运行 LDAP 服务并带可选端口的主机名或 IP。
              如果没指定端口，将使用 LDAP 服务默认的 389 端口。  </td>
          <td>ldap.example.com:389</td>
        </tr>
        <tr class="odd">
          <td>Auth. DN（认证识别名）</td>
          <td>用于认证的识别名。替换单一 %s 的位置为用户名。</td>
      ...
      <h2>ADS 认证</h2>
      <p>       
       这个对话框仅在这种情况可见，即后台被配置成支持 ADS （微软的活动目录服务）认证和授权。
      </p>
      <p>       
       使用这种方式，需要在 ADS 后台做大量工作，比如访问规则和角色存储在 LDAP 里，
       这与 "per-User" 方式相反。建议还是使用 "per-User" 方式。
      </p>
      <p>
       更改将会在点击 “保存” 按钮确认后被保存，但是<b>仅在后台服务重启后才会生效</b>。      
      ...
      <table class="gbntable">
        ...
        <tr class="even">
          <td>ADS Host（ADS主机）</td>
          <td>ADS 主机绑定的地址和可选的端口号。</td>
          <td>ads.example.com:389</td>
        </tr>
        <tr class="odd">
          <td>Domain（域）</td>
          <td>用于认证的域，这个应该包含 TLD（顶级域名）。 </td>
          <td>adstest.local</td>
        </tr>
      </table>
    </div>
...
<xsl:template mode="help" match="tasks.html">
  ...
  <div class="gb_window_part_content">
    ...
    <div style="text-align:left">
      ...
      <h4>暂停任务</h4>
      <p>
       点击暂停图标<img src="/img/pause.png" alt="暂停" title="暂停"/>
       将会暂停一个运行中的任务。任务列表页面将会被更新。
      </p>
      <p>
       这个动作仅在任务状态为“运行中”或“已请求”时才可用，并且该任务不是一个计划任务，也不是一个容器任务。
      </p>
      <p>
       请注意扫描服务将保持在随时待命的活跃状态并将不会释放已分配的内存或进程。
       因此，暂停的任务会对整个性能造成显著影响。
      </p>
